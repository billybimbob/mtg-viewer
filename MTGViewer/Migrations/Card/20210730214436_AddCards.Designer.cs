// <auto-generated />
using System;
using MTGViewer.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace MTGViewer.Migrations.Card
{
    [DbContext(typeof(CardDbContext))]
    [Migration("20210730214436_AddCards")]
    partial class AddCards
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "5.0.7");

            modelBuilder.Entity("CardColor", b =>
                {
                    b.Property<string>("CardsId")
                        .HasColumnType("TEXT");

                    b.Property<string>("ColorsName")
                        .HasColumnType("TEXT");

                    b.HasKey("CardsId", "ColorsName");

                    b.HasIndex("ColorsName");

                    b.ToTable("CardColor");
                });

            modelBuilder.Entity("CardSubType", b =>
                {
                    b.Property<string>("CardsId")
                        .HasColumnType("TEXT");

                    b.Property<string>("SubTypesName")
                        .HasColumnType("TEXT");

                    b.HasKey("CardsId", "SubTypesName");

                    b.HasIndex("SubTypesName");

                    b.ToTable("CardSubType");
                });

            modelBuilder.Entity("CardSuperType", b =>
                {
                    b.Property<string>("CardsId")
                        .HasColumnType("TEXT");

                    b.Property<string>("SuperTypesName")
                        .HasColumnType("TEXT");

                    b.HasKey("CardsId", "SuperTypesName");

                    b.HasIndex("SuperTypesName");

                    b.ToTable("CardSuperType");
                });

            modelBuilder.Entity("CardType", b =>
                {
                    b.Property<string>("CardsId")
                        .HasColumnType("TEXT");

                    b.Property<string>("TypesName")
                        .HasColumnType("TEXT");

                    b.HasKey("CardsId", "TypesName");

                    b.HasIndex("TypesName");

                    b.ToTable("CardType");
                });

            modelBuilder.Entity("MTGViewer.Areas.Identity.Data.CardUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("TEXT");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("INTEGER");

                    b.Property<string>("ConcurrencyStamp")
                        .HasColumnType("TEXT");

                    b.Property<string>("Email")
                        .HasColumnType("TEXT");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("INTEGER");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("INTEGER");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("TEXT");

                    b.Property<string>("Name")
                        .HasColumnType("TEXT");

                    b.Property<string>("NormalizedEmail")
                        .HasColumnType("TEXT");

                    b.Property<string>("NormalizedUserName")
                        .HasColumnType("TEXT");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("TEXT");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("TEXT");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("INTEGER");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("TEXT");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("INTEGER");

                    b.Property<string>("UserName")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("AspNetUsers", t => t.ExcludeFromMigrations());
                });

            modelBuilder.Entity("MTGViewer.Data.Card", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("TEXT");

                    b.Property<string>("Artist")
                        .HasColumnType("TEXT");

                    b.Property<int?>("Cmc")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Flavor")
                        .HasColumnType("TEXT");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("TEXT");

                    b.Property<string>("Layout")
                        .HasColumnType("TEXT");

                    b.Property<string>("Loyalty")
                        .HasColumnType("TEXT");

                    b.Property<string>("ManaCost")
                        .HasColumnType("TEXT");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Power")
                        .HasColumnType("TEXT");

                    b.Property<string>("Rarity")
                        .HasColumnType("TEXT");

                    b.Property<string>("SetName")
                        .HasColumnType("TEXT");

                    b.Property<string>("Text")
                        .HasColumnType("TEXT");

                    b.Property<string>("Toughness")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.ToTable("Cards");
                });

            modelBuilder.Entity("MTGViewer.Data.CardAmount", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Amount")
                        .HasColumnType("INTEGER");

                    b.Property<string>("CardId")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<bool>("IsRequest")
                        .HasColumnType("INTEGER");

                    b.Property<Guid>("LiteToken")
                        .IsConcurrencyToken()
                        .HasColumnType("TEXT");

                    b.Property<int>("LocationId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("CardId");

                    b.HasIndex("LocationId", "CardId", "IsRequest")
                        .IsUnique();

                    b.ToTable("Amounts");
                });

            modelBuilder.Entity("MTGViewer.Data.Color", b =>
                {
                    b.Property<string>("Name")
                        .HasColumnType("TEXT");

                    b.HasKey("Name");

                    b.ToTable("Color");
                });

            modelBuilder.Entity("MTGViewer.Data.Location", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<Guid>("LiteToken")
                        .IsConcurrencyToken()
                        .HasColumnType("TEXT");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("OwnerId")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("OwnerId");

                    b.ToTable("Locations");
                });

            modelBuilder.Entity("MTGViewer.Data.Name", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<string>("CardId")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Value")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("CardId");

                    b.ToTable("Name");
                });

            modelBuilder.Entity("MTGViewer.Data.SubType", b =>
                {
                    b.Property<string>("Name")
                        .HasColumnType("TEXT");

                    b.HasKey("Name");

                    b.ToTable("SubType");
                });

            modelBuilder.Entity("MTGViewer.Data.SuperType", b =>
                {
                    b.Property<string>("Name")
                        .HasColumnType("TEXT");

                    b.HasKey("Name");

                    b.ToTable("SuperType");
                });

            modelBuilder.Entity("MTGViewer.Data.Trade", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Amount")
                        .HasColumnType("INTEGER");

                    b.Property<string>("CardId")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int?>("FromId")
                        .HasColumnType("INTEGER");

                    b.Property<bool>("IsCounter")
                        .HasColumnType("INTEGER");

                    b.Property<Guid>("LiteToken")
                        .IsConcurrencyToken()
                        .HasColumnType("TEXT");

                    b.Property<string>("ProposerId")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("ReceiverId")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("ToId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("FromId")
                        .IsUnique();

                    b.HasIndex("ProposerId");

                    b.HasIndex("ReceiverId");

                    b.HasIndex("ToId");

                    b.HasIndex("CardId", "ProposerId", "ReceiverId", "ToId", "FromId")
                        .IsUnique();

                    b.ToTable("Trades");
                });

            modelBuilder.Entity("MTGViewer.Data.Type", b =>
                {
                    b.Property<string>("Name")
                        .HasColumnType("TEXT");

                    b.HasKey("Name");

                    b.ToTable("Type");
                });

            modelBuilder.Entity("CardColor", b =>
                {
                    b.HasOne("MTGViewer.Data.Card", null)
                        .WithMany()
                        .HasForeignKey("CardsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Data.Color", null)
                        .WithMany()
                        .HasForeignKey("ColorsName")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CardSubType", b =>
                {
                    b.HasOne("MTGViewer.Data.Card", null)
                        .WithMany()
                        .HasForeignKey("CardsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Data.SubType", null)
                        .WithMany()
                        .HasForeignKey("SubTypesName")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CardSuperType", b =>
                {
                    b.HasOne("MTGViewer.Data.Card", null)
                        .WithMany()
                        .HasForeignKey("CardsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Data.SuperType", null)
                        .WithMany()
                        .HasForeignKey("SuperTypesName")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CardType", b =>
                {
                    b.HasOne("MTGViewer.Data.Card", null)
                        .WithMany()
                        .HasForeignKey("CardsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Data.Type", null)
                        .WithMany()
                        .HasForeignKey("TypesName")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("MTGViewer.Data.CardAmount", b =>
                {
                    b.HasOne("MTGViewer.Data.Card", "Card")
                        .WithMany("Amounts")
                        .HasForeignKey("CardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Data.Location", "Location")
                        .WithMany("Cards")
                        .HasForeignKey("LocationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Card");

                    b.Navigation("Location");
                });

            modelBuilder.Entity("MTGViewer.Data.Location", b =>
                {
                    b.HasOne("MTGViewer.Areas.Identity.Data.CardUser", "Owner")
                        .WithMany()
                        .HasForeignKey("OwnerId");

                    b.Navigation("Owner");
                });

            modelBuilder.Entity("MTGViewer.Data.Name", b =>
                {
                    b.HasOne("MTGViewer.Data.Card", "Card")
                        .WithMany("Names")
                        .HasForeignKey("CardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Card");
                });

            modelBuilder.Entity("MTGViewer.Data.Trade", b =>
                {
                    b.HasOne("MTGViewer.Data.Card", "Card")
                        .WithMany()
                        .HasForeignKey("CardId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Data.CardAmount", "From")
                        .WithOne()
                        .HasForeignKey("MTGViewer.Data.Trade", "FromId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("MTGViewer.Areas.Identity.Data.CardUser", "Proposer")
                        .WithMany()
                        .HasForeignKey("ProposerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Areas.Identity.Data.CardUser", "Receiver")
                        .WithMany()
                        .HasForeignKey("ReceiverId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("MTGViewer.Data.Location", "To")
                        .WithMany()
                        .HasForeignKey("ToId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Card");

                    b.Navigation("From");

                    b.Navigation("Proposer");

                    b.Navigation("Receiver");

                    b.Navigation("To");
                });

            modelBuilder.Entity("MTGViewer.Data.Card", b =>
                {
                    b.Navigation("Amounts");

                    b.Navigation("Names");
                });

            modelBuilder.Entity("MTGViewer.Data.Location", b =>
                {
                    b.Navigation("Cards");
                });
#pragma warning restore 612, 618
        }
    }
}

...
public class CreateTests : IAsyncLifetime
{
    ...
    {
        // Arrange
        var wrongUser = await _dbContext.Players.FirstAsync(p => p.Id != _requestDeck.OwnerId);
    ...
    public async Task OnPost_MultipleSources_RequestsAll()
    {
        ...
        var nonOwner = await _dbContext.Players
            .FirstAsync(p => p.Id != _requestDeck.OwnerId);
